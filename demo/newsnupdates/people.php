...
$thissubcat="newsnupdates - lucianpeople";
...
$db = mysql_connect("localhost", "islandsupport", "stpecnoc");
		mysql_select_db("islandsupport",$db);
		$codeget = mysql_query("SELECT * FROM `advertisers` WHERE `section`='$thiscat' ORDER BY `paystatus` ASC, `company` ASC, `id` DESC",$db);
...
$db = mysql_connect("localhost", "islandsupport", "stpecnoc");
	 ...
	 //for ($i=1; $i<=$numpages; $i++) {
		...
		if (isset($_GET['shw'])) {
			...
			$codegetstring.=" ORDER BY `company` ASC, `id` DESC";
			
			$codeget = mysql_query($codegetstring,$db);
		} else {
			$codeget = mysql_query("SELECT * FROM `advertisers` WHERE `section`='$thiscat' AND `subsection`='$thissubcat' ORDER BY `paystatus` ASC, `company` ASC, `id` DESC",$db);
